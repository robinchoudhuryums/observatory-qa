...
export function registerAccessRoutes(app: Express): void {
  ...
  app.post("/api/access-requests", asyncHandler(async (req, res) => {
  ...
  app.get("/api/access-requests", requireAuth, injectOrgContext, requireRole("admin"), asyncHandler(async (req, res) => {
    const requests = await storage.getAllAccessRequests(req.orgId!);
    res.json(requests);
  }));

  // Approve or deny an access request
  app.patch("/api/access-requests/:id", requireAuth, injectOrgContext, requireRole("admin"), asyncHandler(async (req, res) => {
