...
export function registerSpendTrackingRoutes(app: Express): void {
  ...
  app.get("/api/usage", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
  ...
  // --- Cost forecasting ---
  app.get("/api/usage/forecast", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
    const records = await storage.getUsageRecords(req.orgId!);
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
  ...
  // --- Cost per outcome ---
  app.get("/api/usage/cost-per-outcome", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
  ...
  app.get("/api/usage/by-department", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
    const orgId = req.orgId!;
    const records = await storage.getUsageRecords(orgId);
    const filtered = filterRecords(records, req.query);
    const callRecords = filtered.filter((r) => r.type === "call");
  ...
  app.get("/api/usage/anomalies", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
    const records = await storage.getUsageRecords(req.orgId!);
    const filtered = filterRecords(records, req.query);

    if (filtered.length < 5) {
      res.json({ anomalies: [], message: "Need at least 5 records for anomaly detection" });
      return;
    }
  ...
  app.get("/api/usage/budget", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
    const org = await storage.getOrganization(req.orgId!);
    const budgetAlerts = (org?.settings as any)?.budgetAlerts || { enabled: false };
    res.json(budgetAlerts);
  }));

  app.put("/api/usage/budget", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
    const orgId = req.orgId!;
    const { enabled, monthlyBudgetUsd, alertEmail } = req.body;

    if (enabled && (!monthlyBudgetUsd || monthlyBudgetUsd <= 0)) {
      throw new AppError(400, "monthlyBudgetUsd must be a positive number when budget alerts are enabled");
    }
