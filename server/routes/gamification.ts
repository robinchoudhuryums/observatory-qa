...
export async function recordActivity(
  orgId: string,
  employeeId: string,
  pointType: keyof typeof POINT_VALUES,
): Promise<void> {
  try {
    const profile = await storage.getGamificationProfile(orgId, employeeId);
...
export function registerGamificationRoutes(app: Express) {
  ...
  app.get("/api/gamification/leaderboard", requireAuth, injectOrgContext, asyncHandler(async (req, res) => {
      ...
      const employeeResults = await Promise.all(neededIds.map((id) => storage.getEmployee(orgId, id)));
      const employeeMap = new Map(employeeResults.filter(Boolean).map((e) => [e!.id, e!]));
  ...
  app.get(
    "/api/gamification/profile/:employeeId",
    requireAuth,
    injectOrgContext,
  ...
  app.put("/api/gamification/settings", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
      ...
      const gamification = {
        ...
        optedOutEmployeeIds: Array.isArray(optedOutEmployeeIds)
          ? optedOutEmployeeIds
          : settings.gamification?.optedOutEmployeeIds,
  ...
  app.post("/api/gamification/recognize", requireAuth, requireRole("manager"), injectOrgContext, asyncHandler(async (req, res) => {
      ...
      logger.info(
        { orgId, employeeId, badgeId: customBadgeId, awardedBy: req.user!.id },
  ...
  app.get("/api/gamification/team-leaderboard", requireAuth, injectOrgContext, asyncHandler(async (req, res) => {
      ...
      const teams: Record<
        string,
        { totalPoints: number; memberCount: number; avgPoints: number; topPerformer: string | null; badges: number }
      > = {};
  ...
  app.get("/api/gamification/effectiveness", requireAuth, requireRole("admin"), injectOrgContext, asyncHandler(async (req, res) => {
      ...
      res.json({
        ...
        interpretation:
          correlation > 0.5
            ? "Strong positive correlation — badges correlate with higher performance"
            : correlation > 0.2
              ? "Moderate positive correlation"
              : correlation > -0.2
                ? "No significant correlation between badges and performance"
        ...
        comparison: {
          ...
          scoreDifference:
            avgScoreHighBadge !== null && avgScoreLowBadge !== null
