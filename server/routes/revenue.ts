...
export function registerRevenueRoutes(app: Express) {
  ...
  app.get("/api/revenue/attribution", requireAuth, injectOrgContext, asyncHandler(async (req, res) => {
      ...
      const rates = {
        callToAppointment:
          funnel.callIdentified > 0
            ? Math.round((funnel.appointmentScheduled / funnel.callIdentified) * 10000) / 100
            : 0,
        appointmentToCompletion:
          funnel.appointmentScheduled > 0
            ? Math.round((funnel.appointmentCompleted / funnel.appointmentScheduled) * 10000) / 100
            : 0,
        completionToTreatment:
          funnel.appointmentCompleted > 0
            ? Math.round((funnel.treatmentAccepted / funnel.appointmentCompleted) * 10000) / 100
            : 0,
        treatmentToPayment:
          funnel.treatmentAccepted > 0
            ? Math.round((funnel.paymentCollected / funnel.treatmentAccepted) * 10000) / 100
            : 0,
        overallConversion:
          funnel.callIdentified > 0 ? Math.round((funnel.paymentCollected / funnel.callIdentified) * 10000) / 100 : 0,
      };
        ...
        scheduled:
          Math.round(
            revenues
              ...
              .reduce((s, r) => s + (r.treatmentValue || r.estimatedRevenue || 0), 0) * 100,
          ) / 100,
        collected:
          Math.round(revenues.reduce((s, r) => s + (r.paymentCollected || r.actualRevenue || 0), 0) * 100) / 100,
      };
  ...
  app.get("/api/revenue/payer-mix", requireAuth, injectOrgContext, asyncHandler(async (req, res) => {
      ...
      // Overall payer mix
      const payerCounts: Record<
        string,
        { count: number; totalRevenue: number; insuranceRevenue: number; patientRevenue: number }
      > = {};
      for (const rev of revenues) {
        const payer = rev.payerType || "unknown";
        if (!payerCounts[payer])
          payerCounts[payer] = { count: 0, totalRevenue: 0, insuranceRevenue: 0, patientRevenue: 0 };
      ...
      for (const rev of revenues) {
        ...
        const normalized = rev.insuranceCarrier
          .trim()
      ...
      const byEmployee: Record<
        string,
        { employeeName: string; insurance: number; cash: number; mixed: number; unknown: number; total: number }
      > = {};
      for (const rev of revenues) {
        const call = callMap.get(rev.callId);
        if (!call?.employeeId) continue;
      ...
      res.json({
        ...
        byEmployee: Object.values(byEmployee)
          .map((e) => ({
            ...e,
            insurance: Math.round(e.insurance * 100) / 100,
