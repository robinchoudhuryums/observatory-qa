...
export function registerEhrRoutes(app: Express): void {
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if (err instanceof EhrError) {
          const status =
            err.errorType === "auth" ? 502 : err.errorType === "network" || err.errorType === "timeout" ? 504 : 502;
  ...
  app.get(
    "/api/ehr/patients/:ehrPatientId/treatment-plans",
    requireAuth,
    injectOrgContext,
    asyncHandler(async (req, res) => {
      const org = await getCachedOrganization(req.orgId!);
  ...
  app.get(
    "/api/ehr/patients/:ehrPatientId/prefill-data",
    requireAuth,
    injectOrgContext,
    asyncHandler(async (req, res) => {
      const org = await getCachedOrganization(req.orgId!);
  ...
  app.post(
    "/api/ehr/push-note/:callId/retry",
    requireAuth,
    injectOrgContext,
    requireRole("manager", "admin"),
    asyncHandler(async (req, res) => {
  ...
  app.get(
    "/api/ehr/match-appointment/:callId",
    requireAuth,
    injectOrgContext,
    asyncHandler(async (req, res) => {
  ...
  app.patch(
    "/api/ehr/treatment-plans/:planId",
    requireAuth,
    injectOrgContext,
    requireRole("manager", "admin"),
    asyncHandler(async (req, res) => {
  ...
  app.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!secretArn.startsWith("arn:aws:secretsmanager:")) {
        res.status(400).json({
          message:
            "Invalid Secrets Manager ARN format. Expected: arn:aws:secretsmanager:{region}:{account}:secret:{name}",
        });
        return;
      }
