    ...
    async (req, res) => {
      try {
        ...
        if (!cn.providerAttested) {
          res.status(403).json({
            message:
              "Clinical note must be attested before FHIR export. Please have the provider attest the note first.",
      ...
      try {
        ...
        if (!clinicalRows) {
          res.json({
            ...
            avgPainScale: null,
            avgBmi: null,
            avgBloodPressureSystolic: null,
            avgBloodPressureDiastolic: null,
            topIcd10Codes: [],
            topMedications: [],
            vitalsPresent: 0,
            medicationsPresent: 0,
          });
          return;
        }
        ...
        let painScaleTotal = 0;
        let painScaleCount = 0;
        let bmiTotal = 0;
        let bmiCount = 0;
        let bpSysTotal = 0;
        let bpSysCount = 0;
        ...
        for (const n of notesWithData) {
          ...
          if (sd?.vitals) {
            vitalsPresent++;
            if (typeof sd.vitals.painScale === "number") {
              painScaleTotal += sd.vitals.painScale;
              painScaleCount++;
            }
            if (typeof sd.vitals.bmi === "number") {
              bmiTotal += sd.vitals.bmi;
              bmiCount++;
            }
            if (typeof sd.vitals.bloodPressureSystolic === "number") {
              bpSysTotal += sd.vitals.bloodPressureSystolic;
              bpSysCount++;
            }
            if (typeof sd.vitals.bloodPressureDiastolic === "number") {
              bpDiaTotal += sd.vitals.bloodPressureDiastolic;
              bpDiaCount++;
            }
          }
