...
export class MemStorage implements IStorage {
  ...
  async getUsersByUsername(username: string): Promise<User[]> {
    return Array.from(this.users.values()).filter((u) => u.username === username);
  }
  ...
  async listCallRevenues(
    ...
    if (filters?.startDate) {
      ...
      results = results.filter((r) => r.createdAt && new Date(r.createdAt).getTime() >= start);
    }
    if (filters?.endDate) {
      const end = new Date(filters.endDate).getTime();
      results = results.filter((r) => r.createdAt && new Date(r.createdAt).getTime() <= end);
    }
    return results.sort((a, b) => (b.createdAt || "").localeCompare(a.createdAt || ""));
  }
