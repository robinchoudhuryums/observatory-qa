...
(async () => {
  ...
  server.listen({
    ...
    const runCoachingScheduledTasks = async () => {
      try {
        const { runAutomationRules, sweepEffectivenessSnapshots, getDueSoonSessions, getOverdueSessions } = await import("./services/coaching-engine");
        ...
        for (const org of orgs) {
          ...
          try {
            ...
            if (sessionsCreated > 0) logger.info({ orgId: org.id, triggered, sessionsCreated }, "Automation rules created coaching sessions");
            ...
            if (dueSoon.length > 0) {
              ...
              for (const [manager, items] of Array.from(byManager)) {
                const list = items.map(i => `• ${i.session.title} (${i.employeeName}, due in ${i.hoursUntilDue}h)`).join("\n");
