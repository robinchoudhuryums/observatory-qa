/**
 * Observatory SVG logo component with inline SVG for full CSS control.
 * Supports: dark/light mode (via currentColor), custom colors, icon-only variant,
 * hover transitions, loading animation, and animated entrance.
 */
import { useState } from "react";
import { cn } from "@/lib/utils";

interface ObservatoryLogoProps {
  /** "full" = icon + text, "icon" = icon only */
  variant?: "full" | "icon";
  /** Height in pixels (width scales proportionally) */
  height?: number;
  /** Override fill color (defaults to currentColor for theme auto-switching) */
  color?: string;
  /** Show pulsing animation (for loading states) */
  animated?: boolean;
  /** Enable hover color transition */
  hoverable?: boolean;
  /** Hover color (defaults to brand teal) */
  hoverColor?: string;
  className?: string;
}

/** Shared owl icon SVG paths */
const OWL_PATHS = (
  <>
    <path d="M60.146,32.33c-3.833,0-6.951,3.118-6.951,6.951c0,0.51,0.059,1.005,0.163,1.484c-2.886-1.626-5.3-1.021-6.685-0.141c0.085-0.435,0.133-0.884,0.133-1.344c0-3.833-3.118-6.951-6.951-6.951c-3.833,0-6.951,3.118-6.951,6.951s3.118,6.951,6.951,6.951c2.015,0,3.826-0.867,5.097-2.241c0.03,0.038,0.044,0.084,0.079,0.119l4.086,4.086c0.234,0.234,0.552,0.366,0.884,0.366s0.649-0.132,0.884-0.366l4.085-4.086c0.035-0.035,0.049-0.081,0.079-0.119c1.271,1.374,3.083,2.241,5.097,2.241c3.833,0,6.951-3.118,6.951-6.951S63.978,32.33,60.146,32.33z M39.855,43.732c-2.455,0-4.451-1.997-4.451-4.451s1.997-4.451,4.451-4.451c2.454,0,4.451,1.997,4.451,4.451S42.309,43.732,39.855,43.732z M50,45.545l-2.452-2.452c0.032-0.026,0.071-0.038,0.101-0.067c0.185-0.186,1.871-1.728,4.776,0.094L50,45.545z M60.146,43.732c-2.455,0-4.451-1.997-4.451-4.451s1.997-4.451,4.451-4.451c2.454,0,4.451,1.997,4.451,4.451S62.6,43.732,60.146,43.732z" />
    <path d="M27.054,59.49c0.367,5.874,2.454,10.489,6.202,13.718c2.772,2.388,6.019,3.64,8.892,4.294v1.324c0,0.69,0.56,1.25,1.25,1.25s1.25-0.56,1.25-1.25v-0.875c0.293,0.038,0.579,0.071,0.854,0.099v0.776c0,0.69,0.56,1.25,1.25,1.25s1.25-0.56,1.25-1.25v-0.641c0.067,0.001,0.143,0.003,0.207,0.003c0.822,0,1.338-0.041,1.425-0.049c0.132-0.012,0.25-0.059,0.366-0.107c0.116,0.049,0.234,0.096,0.366,0.107c0.086,0.008,0.603,0.049,1.425,0.049c0.064,0,0.14-0.002,0.207-0.003v0.641c0,0.69,0.56,1.25,1.25,1.25s1.25-0.56,1.25-1.25V78.05c0.275-0.027,0.56-0.061,0.854-0.099v0.875c0,0.69,0.56,1.25,1.25,1.25s1.25-0.56,1.25-1.25v-1.323c2.873-0.654,6.12-1.907,8.892-4.295c3.748-3.229,5.835-7.844,6.202-13.718c0.277-4.437,0.014-17.334,0.002-17.854c0-11.972-10.229-21.712-22.802-21.712c-0.052,0-0.096,0.023-0.146,0.029c-0.05-0.006-0.095-0.029-0.146-0.029c-12.573,0-22.802,9.74-22.801,21.686C27.041,42.156,26.777,55.054,27.054,59.49z M70.451,59.334c-0.262,4.194-1.507,7.621-3.676,10.251c-0.109-0.04-0.218-0.084-0.34-0.092c-1.671-0.107-2.934-0.716-3.861-1.861c-2.504-3.095-1.677-8.943-1.669-9.002c0.101-0.683-0.371-1.318-1.053-1.419c-0.69-0.111-1.318,0.368-1.42,1.051c-0.042,0.283-1,6.981,2.192,10.936c1.029,1.275,2.356,2.122,3.958,2.529c-2.104,1.648-4.507,2.628-6.731,3.19v-2.471c0-0.69-0.56-1.25-1.25-1.25s-1.25,0.56-1.25,1.25v2.963c-0.292,0.042-0.579,0.081-0.854,0.111v-3.074c0-0.69-0.56-1.25-1.25-1.25s-1.25,0.56-1.25,1.25v3.232c-0.865,0.012-1.397-0.028-1.417-0.029c-0.21-0.02-0.405,0.029-0.586,0.105c-0.178-0.073-0.37-0.12-0.577-0.105c-0.021,0.001-0.551,0.042-1.416,0.032v-3.235c0-0.69-0.56-1.25-1.25-1.25s-1.25,0.56-1.25,1.25v3.079c-0.274-0.03-0.562-0.068-0.854-0.111v-2.968c0-0.69-0.56-1.25-1.25-1.25s-1.25,0.56-1.25,1.25v2.478c-2.221-0.563-4.623-1.545-6.731-3.197c1.602-0.407,2.929-1.254,3.958-2.529c3.193-3.954,2.235-10.652,2.192-10.936c-0.102-0.683-0.735-1.155-1.421-1.052c-0.683,0.103-1.153,0.738-1.051,1.421c0.244,1.628,0.419,6.416-1.665,8.996c-0.927,1.148-2.191,1.759-3.865,1.866c-0.122,0.008-0.23,0.052-0.34,0.092c-2.168-2.63-3.414-6.057-3.676-10.251c-0.189-3.025-0.114-10.374-0.051-14.59c2.845,6.102,10.688,10.387,20.182,10.387c0.113,0,0.215-0.036,0.319-0.064c0.104,0.028,0.206,0.064,0.319,0.064c9.494,0,17.337-4.285,20.183-10.387C70.565,48.949,70.64,56.306,70.451,59.334z M49.854,22.424c0.052,0,0.096-0.023,0.146-0.029c0.05,0.006,0.095,0.029,0.146,0.029c5.287,0,10.093,1.938,13.709,5.086c-2.636-0.979-5.892-0.981-8.674,0.063c-2.286,0.858-4.043,2.326-5.182,4.276c-1.139-1.95-2.896-3.419-5.183-4.276c-2.785-1.046-6.043-1.041-8.678-0.062C39.756,24.364,44.564,22.424,49.854,22.424z M50.319,52.631c-0.113,0-0.215,0.036-0.319,0.064c-0.104-0.028-0.206-0.064-0.319-0.064c-10.567,0-18.845-5.701-18.845-12.979c0-4.113,2.08-8.758,6.727-9.861c0.187-0.044,0.347-0.135,0.487-0.247c1.872-0.435,4.036-0.326,5.888,0.369c1.518,0.57,3.527,1.796,4.481,4.401c0.186,0.507,0.664,0.82,1.174,0.82c0.135,0,0.272-0.025,0.407-0.072c0.135,0.047,0.272,0.072,0.407,0.072c0.51,0,0.988-0.313,1.174-0.82c0.954-2.605,2.962-3.831,4.479-4.4c1.853-0.696,4.017-0.805,5.889-0.37c0.14,0.112,0.3,0.203,0.487,0.247c4.647,1.104,6.727,5.748,6.727,9.861C69.164,46.93,60.886,52.631,50.319,52.631z" />
  </>
);

/** "Observatory" text SVG path */
const TEXT_PATH =
  "M2.34 12.92 q0 2.7 1.66 4.45 t4.34 1.79 q2.7 0 4.34 -1.77 t1.66 -4.47 q0 -2.74 -1.64 -4.47 t-4.38 -1.77 q-2.68 0 -4.32 1.76 t-1.66 4.48 z M8.34 5.48 q2.16 0 3.82 0.95 t2.57 2.66 t0.93 3.83 q0 3.26 -2.06 5.33 t-5.26 2.11 q-3.22 0 -5.24 -2.09 t-2.08 -5.35 q0.02 -3.26 2.04 -5.32 t5.28 -2.12 z M19.1 15.38 q0 1.6 1.15 2.67 t2.71 1.11 q1.62 0 2.64 -1.09 t1.02 -2.69 q0 -1.64 -1.03 -2.71 t-2.65 -1.07 q-1.58 0 -2.69 1.09 t-1.15 2.69 z M18.02 4.88 l1.2 0 l0 7.5 l0.04 0 q0.52 -0.9 1.54 -1.38 t2.16 -0.48 q2.16 0 3.51 1.4 t1.35 3.46 q0 2.14 -1.38 3.49 t-3.48 1.37 q-1.18 0 -2.21 -0.52 t-1.49 -1.34 l-0.04 0 l0 1.62 l-1.2 0 l0 -15.12 z M33.02 10.52 q2.34 0 3.24 1.74 l-1.08 0.64 q-0.68 -1.3 -2.16 -1.3 q-0.8 0 -1.39 0.43 t-0.59 1.05 t0.5 0.99 t1.74 0.59 q1.84 0.32 2.54 0.95 t0.7 1.77 q0 1.3 -0.93 2.08 t-2.43 0.78 q-2.64 -0.02 -3.7 -1.88 l1.06 -0.72 q0.34 0.7 1.11 1.11 t1.57 0.41 q0.92 0 1.52 -0.51 t0.6 -1.19 q0 -0.62 -0.49 -1.02 t-2.13 -0.72 q-1.42 -0.28 -2.14 -0.86 t-0.72 -1.68 q0 -1.18 0.9 -1.92 t2.28 -0.74 z M46.26 14.6 q0 -1.26 -0.94 -2.13 t-2.28 -0.87 q-1.3 0 -2.24 0.87 t-1.1 2.13 l6.56 0 z M43.12 10.52 q1.92 0 3.12 1.24 t1.22 3.2 l0 0.72 l-7.76 0 q0.08 1.56 1.03 2.51 t2.47 0.97 q0.94 0 1.79 -0.45 t1.31 -1.19 l0.88 0.74 q-1.3 1.98 -4 1.98 q-2.12 -0.02 -3.39 -1.38 t-1.29 -3.5 q0 -2.08 1.31 -3.45 t3.31 -1.39 z M54.26 10.52 q0.34 0 0.7 0.1 l-0.14 1.18 q-0.22 -0.08 -0.48 -0.08 q-1.46 0 -2.23 0.9 t-0.77 2.34 l0 5.04 l-1.2 0 l0 -6.74 q0 -0.2 -0.1 -2.5 l1.2 0 q0.04 1.38 0.08 1.62 q1.04 -1.86 2.94 -1.86 z M56.86 10.76 l3.08 7.94 l3 -7.94 l1.22 0 l-3.64 9.24 l-1.24 0 l-3.78 -9.24 l1.36 0 z M66.84 17.44 q0 0.76 0.61 1.24 t1.51 0.48 q1.62 0 2.41 -0.85 t0.79 -2.31 l0 -0.64 l-1.44 0 q-1.84 0 -2.86 0.56 t-1.02 1.52 z M69.56 10.52 q1.86 0 2.76 0.86 t0.92 2.4 l0 2.74 q0 2.24 0.18 3.48 l-1.12 0 q-0.12 -0.62 -0.12 -1.5 l-0.04 0 q-1 1.74 -3.22 1.74 q-1.5 0 -2.38 -0.74 t-0.9 -2 q0 -1.54 1.33 -2.32 t3.71 -0.78 l1.48 0 l0 -0.54 q0 -1.14 -0.7 -1.7 t-1.9 -0.56 q-1.6 0 -2.82 1.04 l-0.7 -0.82 q0.66 -0.64 1.63 -0.97 t1.89 -0.33 z M77.12 8.1 l1.2 0 l0 2.66 l2.64 0 l0 1.08 l-2.64 0 l0 6.04 q0 0.58 0.34 0.93 t0.94 0.35 q0.66 0 1.36 -0.32 l0.1 1.08 q-0.9 0.32 -1.58 0.32 q-1.18 0 -1.77 -0.66 t-0.59 -1.7 l0 -6.04 l-2 0 l0 -1.08 l2 0 l0 -2.66 z M83.78 15.38 q0 1.66 1.04 2.72 t2.62 1.06 q1.64 0 2.65 -1.08 t1.01 -2.7 q0 -1.6 -1 -2.68 t-2.68 -1.1 q-1.62 0 -2.62 1.08 t-1.02 2.7 z M87.44 10.52 q2.12 0 3.48 1.38 t1.38 3.48 q0 2.14 -1.39 3.49 t-3.47 1.37 q-2.18 -0.02 -3.51 -1.4 t-1.35 -3.46 q0 -2.14 1.37 -3.49 t3.49 -1.37 z M99.1 10.52 q0.34 0 0.7 0.1 l-0.14 1.18 q-0.22 -0.08 -0.48 -0.08 q-1.46 0 -2.23 0.9 t-0.77 2.34 l0 5.04 l-1.2 0 l0 -6.74 q0 -0.2 -0.1 -2.5 l1.2 0 q0.04 1.38 0.08 1.62 q1.04 -1.86 2.94 -1.86 z M101.68 10.76 l3.06 7.78 l2.98 -7.78 l1.28 0 l-4.74 12.06 q-0.78 1.98 -2.56 1.98 q-0.56 0 -1.18 -0.18 l0.12 -1.12 q0.66 0.22 1.08 0.22 q0.46 0 0.82 -0.32 t0.7 -1.16 l0.86 -2.2 l-3.76 -9.28 l1.34 0 z";

/** Owl icon SVG (icon-only variant) */
function OwlIcon({ size = 36, color, animated }: { size?: number; color?: string; animated?: boolean }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 80 80"
      fill={color || "currentColor"}
      width={size}
      height={size}
      aria-hidden="true"
      className={animated ? "animate-pulse" : undefined}
    >
      <g transform="translate(3, -12) scale(0.78)">{OWL_PATHS}</g>
    </svg>
  );
}

export function ObservatoryLogo({
  variant = "full",
  height = 36,
  color,
  animated = false,
  hoverable = false,
  hoverColor = "#14b8a6",
  className,
}: ObservatoryLogoProps) {
  const [hovered, setHovered] = useState(false);
  const resolvedColor = hovered && hoverable ? hoverColor : color;

  const wrapperProps = hoverable
    ? {
        onMouseEnter: () => setHovered(true),
        onMouseLeave: () => setHovered(false),
        style: { transition: "color 200ms ease, fill 200ms ease" } as React.CSSProperties,
      }
    : {};

  if (variant === "icon") {
    return (
      <span
        className={cn(hoverable && "cursor-pointer", className)}
        role="img"
        aria-label="Observatory"
        {...wrapperProps}
      >
        <OwlIcon size={height} color={resolvedColor} animated={animated} />
      </span>
    );
  }

  // Full logo: icon + text, aspect ratio ~3.73:1 from original viewBox
  const width = Math.round(height * 3.73);

  return (
    <span
      className={cn(hoverable && "cursor-pointer", className)}
      role="img"
      aria-label="Observatory"
      {...wrapperProps}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 390 105"
        fill={resolvedColor || "currentColor"}
        width={width}
        height={height}
        aria-hidden="true"
        className={animated ? "animate-pulse" : undefined}
        style={hoverable ? { transition: "fill 200ms ease" } : undefined}
      >
        {/* Owl icon */}
        <g transform="scale(1.4046) translate(-27,-20)">{OWL_PATHS}</g>
        {/* "Observatory" text */}
        <g transform="translate(82, 3) scale(2.641)">
          <path d={TEXT_PATH} />
        </g>
      </svg>
    </span>
  );
}
