...
- **Account lockout eviction preserves locked accounts**: When the `loginAttempts` map is at capacity, eviction skips locked accounts and removes the most stale unlocked entry instead. Prevents attackers from flooding unique usernames to reset real lockouts
- **Stripe webhook events are deduplicated**: The billing webhook handler tracks processed event IDs in memory (24h TTL). Duplicate event deliveries from Stripe are skipped before any state mutations. In multi-instance deployments, this should be moved to Redis
